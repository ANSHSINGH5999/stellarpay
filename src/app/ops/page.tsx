...
import {
  ...
  Search, ShieldCheck, Target, Users, Wifi, WifiOff, Zap,
} from 'lucide-react';
    ...
    <div className="flex flex-col gap-8 animate-fade-in">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-brand-800/50 bg-gradient-to-br from-[#0a1628] via-[#0f1f3d] to-[#0f172a] p-8">
        ...
        <div className="relative flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
            {[
              { label: 'Verified users', value: `${verifiedUsers.length}/30`, icon: Users,       color: 'text-brand-400',   bg: 'bg-brand-500/10',   ring: 'ring-brand-500/20' },
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card">
          ...
          <div className="mt-3 flex items-center gap-2 rounded-2xl border border-emerald-500/25 bg-emerald-500/8 p-3 text-sm text-emerald-200">
            ...
            {verifiedUsers.length}+ verified wallets · Black Belt requirement satisfied
          </div>
        </div>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card">
          ...
          <div className="rounded-2xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/10 to-transparent p-4 mb-4">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" />
              <span className="font-semibold text-emerald-200 text-sm">Fee sponsorship — fee-bump transactions</span>
            </div>
            <p className="text-sm text-slate-300 leading-relaxed">
              StellarPay includes a helper that builds a fee-bump envelope so a sponsor
              account can cover network fees on behalf of the sender — the foundation for
...
buildSponsoredPayment({
  senderSecret,    // signs inner tx
  sponsorSecret,   // pays the fee
  receiverPublic,
  amountXLM,
  memo?,
})
// → { innerXDR, feeBumpXDR, feeSource, ... }`}
          </pre>

          <div className="space-y-2.5 text-sm">
            {[
              { icon: CheckCircle2, color: 'text-emerald-400', text: 'Inner tx signed by sender, outer fee-bump signed by sponsor.' },
              { icon: CheckCircle2, color: 'text-emerald-400', text: 'Returns both XDR envelopes + explorer-ready metadata.' },
