      ...
      {transactions.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {(() => {
            ...
            const sentInr  = xlmToInr(sent);
            const recvInr  = xlmToInr(received);
            return [
              { label: 'Total Sent',     value: sent.toFixed(2) + ' XLM',   sub: '≈ ₹' + sentInr.toFixed(0), color: 'text-red-400'     },
              { label: 'Total Received', value: received.toFixed(2) + ' XLM', sub: '≈ ₹' + recvInr.toFixed(0), color: 'text-emerald-400' },
              { label: 'Net Flow',       value: (net >= 0 ? '+' : '') + net.toFixed(2) + ' XLM', sub: '', color: net >= 0 ? 'text-emerald-400' : 'text-red-400' },
              { label: 'Transactions',   value: String(transactions.length),  sub: filter !== 'all' ? `(${transactions.filter(t => t.type === filter).length} filtered)` : '', color: 'text-brand-400'   },
            ];
